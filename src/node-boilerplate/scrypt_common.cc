...
namespace {
	...
	//
	const char* InternalErrorDescr(const int error) {
		switch(error) {
			case 0:
				return "success";
			case 1:
	...
	const char* ScryptErrorDescr(const int error) {
		std::string scrypt_err_description = InternalErrorDescr(error);

		//if (error) {
		//	const char* c_err_description = strerror(errno);
		//	if (c_err_description) scrypt_err_description; 
		//}
