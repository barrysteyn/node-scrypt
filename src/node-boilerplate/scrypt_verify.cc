...
struct HashInfo {
	//Encodings
	Nan::Encoding hashEncoding, keyEncoding;

	//Async callback function
	Persistent<Function> callback;
	Persistent<Value> hash, key;
	...
	void cleanUp() {
		...
		NanDisposePersistent(hash);
	}

	HashInfo(Handle<Object> config) : hash_ptr(NULL), key_ptr(NULL), keySize(0) { 
		hashEncoding = static_cast<Nan::Encoding>(config->GetHiddenValue(NanNew<String>("_hashEncoding"))->ToUint32()->Value());
	...
	for (int i=0; i < args.Length(); i++) {
		...
		switch(i) {
			case 0: //Hash
				...
				NanAssignPersistent(hashInfo.hash, currentVal);
...
VerifySyncAfterWork(Local<Value>& result, const HashInfo* hashInfo) {
	if (hashInfo->result && hashInfo->result != 11) {
		...
		result = NanNew<Boolean>(hashInfo->result == 0);
	}
}
...
VerifyAsyncAfterWork(uv_work_t* req) {
	...
	Local<Value> argv[2] = {
		Internal::MakeErrorObject(SCRYPT,hashInfo->result),
		NanNew<Boolean>(hashInfo->result == 0)
	};
...
NAN_METHOD(Verify) {
	...
	HashInfo* hashInfo = new HashInfo(Local<Object>::Cast(args.Holder()->Get(NanNew<String>("config"))));
	Local<Value> result;
	...
	//If synchronous, then clear up memory now
	if (hashInfo->callback.IsEmpty()) {
		delete hashInfo;
...
NAN_METHOD(CreateVerifyFunction) {
	...
	verify->Set(NanNew<String>("config"), CreateScryptConfigObject("verify"), v8::ReadOnly);

	NanReturnValue(NanNew(verify)->NewInstance());
}
